...
public class LandleafView extends View {
    ...
    protected void initAttrs(Context context, AttributeSet attrs) {
        ...
        cRadius = ta.getInteger(R.styleable.LandleafView_cRadius, 200);
        cLineWidth = ta.getInteger(R.styleable.LandleafView_cLineWidth, 20);
        cTriangleMargin = ta.getInteger(R.styleable.LandleafView_cTriangleMargin, 10);
        float cTriangleWidth = ta.getInteger(R.styleable.LandleafView_cTriangleWidth, 20);
        //新加入的属性-大小
        progressTextSize = ta.getInteger(R.styleable.LandleafView_cProgressTextSize, 110);

        rangeSize = ta.getInteger(R.styleable.LandleafView_cRangeTextSize, 28);
        txtPaint.setTextSize(rangeSize);

        levelBg = ta.getInteger(R.styleable.LandleafView_cLevelTextBg, Color.parseColor("#39b54a"));
        levelTextSize = ta.getInteger(R.styleable.LandleafView_cLevelTextSize, 28);

        boundSize = ta.getInteger(R.styleable.LandleafView_cBoundSize, 5);
...
//    public void setProgress(float value, float minValue, float maxValue, int color, String levelText) {
    public void setProgress(Progress pro) {
        float value = pro.getValue();
        float maxValue = pro.getMaxValue();
        float minValue = pro.getMinValue();
        ...
        int progress = (int) ((value - minValue) / (maxValue - minValue) * 100.0F);
        this.levelText = pro.getLevelText();
        this.levelBg = pro.getColor();
        ...
        //圆环动画
        valueAnimator(cCurProgress, progress, pro.getColor());
        //文字动画
        runFloat(1500, value);
    }
     ...
     */
    protected void valueAnimator(final float oldProgress, final float endProgress, final int color) {
    ...
    protected void runFloat(int duration, float number) {
        ...
        txtAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
